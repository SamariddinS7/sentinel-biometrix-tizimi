...
export class CrowdAnalyzerPlugin extends BaseAiPlugin {
  public metadata: AiPluginMetadata = {
    id: 'core.crowd_analyzer',
    name: 'Crowd Analysis Module',
    version: '1.0.8',
    vendor: 'Sentinel Biometrik',
    supportedDevices: ['CPU', 'CUDA', 'TENSOR_RT', 'ONNX_RUNTIME'],
    description: 'Measures structural spatial density, estimated group size, and flow direction vectors.'
  };

  private modelFilePath = path.join(process.cwd(), 'models', 'weights', 'crowdnet.onnx');
  ...
  protected async onExecuteInference(frame: VideoFrame): Promise<DynamicDetectionPayload> {
    return {
      ...
      frameId: frame.id,
      metadata: { headCount: 0, density: 'LOW' } // Production real: Run CrowdNet point density matrix mapping
    };
  }
