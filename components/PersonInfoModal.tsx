...
export const PersonInfoModal: React.FC<PersonInfoModalProps> = ({ person, onClose }) => {
    ...
    const roleColor = person.role === UserRole.ADMIN ? 'text-purple-400' : 
                      person.role === UserRole.OPERATOR ? 'text-blue-400' : 'text-slate-400';

    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div 
                className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 relative"
            ...
            >
                ...
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 text-slate-400 hover:text-white z-10 p-1 bg-black/20 rounded-full hover:bg-black/50 transition-colors"
                >
                    <X size={20} />
                </button>
                ...
                <div className="pt-12 px-6 pb-6 relative z-0 flex flex-col items-center">
                    ...
                    <div className="relative mb-4">
                        <div className={`w-24 h-24 rounded-full p-1 border-4 ${isUnknown ? 'border-red-500 bg-red-950' : 'border-cyan-500 bg-slate-900'}`}>
                            {person.avatarUrl ? (
                                <img src={person.avatarUrl} alt={person.name} className="w-full h-full rounded-full object-cover" />
                            ) : (
                                <div className="w-full h-full rounded-full flex items-center justify-center text-slate-500">
                                    <UserIcon size={40} />
                                </div>
                            )}
                        </div>
                    ...
                    <div className="flex items-center gap-2 mb-6">
                        <span className={`text-xs font-mono px-2 py-0.5 rounded border bg-slate-800 border-slate-700 ${roleColor}`}>
                            {person.role}
                        </span>
                        {person.department && (
                    ...
                    <div className="w-full grid grid-cols-2 gap-3 mb-6">
                        <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-mono text-slate-200 truncate max-w-[100px]">{person.id}</p>
                            </div>
                        </div>
                        <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium text-slate-200">{person.status || 'Active'}</p>
                            </div>
                        </div>
                        <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] text-slate-500 uppercase font-bold">Last Seen</p>
                                <p className="text-sm font-medium text-slate-200">{person.lastSeen || 'Just now'}</p>
                            </div>
                        </div>
                        <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 flex items-center gap-3">
                            <div className="p-2 rounded-lg bg-slate-900 text-rose-400">
                                <MapPin size={18} />
                            </div>
                            <div>
                                <p className="text-[10px] text-slate-500 uppercase font-bold">Location</p>
                                <p className="text-sm font-medium text-slate-200">{person.location || 'Unknown'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full flex gap-3">
                        <button className="flex-1 py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-bold transition-colors">
                            View History
                        </button>
