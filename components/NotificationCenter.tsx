...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="absolute top-16 right-4 w-96 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 flex flex-col max-h-[calc(100vh-100px)] animate-in slide-in-from-top-2 fade-in duration-200 origin-top-right">
      {/* Header */}
      <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950 rounded-t-xl">
        <div className="flex items-center gap-2">
          <Bell size={18} className="text-slate-400" />
          <h3 className="font-semibold text-slate-200">Xabarnomalar</h3>
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map((notification) => (
            ...
            >
              ...
              {/* Actions on hover */}
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-slate-900/80 backdrop-blur rounded p-0.5 shadow-sm">
                {!notification.read && (
                  <button 
