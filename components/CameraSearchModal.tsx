...
export const CameraSearchModal: React.FC<CameraSearchModalProps> = ({ isOpen, onClose, cameras, onCameraSelect }) => {
    ...
    const runSearch = async () => {
        ...
        for (const cam of onlineCameras) {
            // For mock cameras, assume they are using known URLs
            const url = cam.id === 'CAM-02' 
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in" onClick={onClose}>
            <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50 rounded-t-xl">
                    ...
                    <button onClick={onClose} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-300 transition-colors">
                        <X size={16} />
                    </button>
                </div>
                ...
                <div className="p-6 flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-6">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-4 pl-12 text-white placeholder-slate-500 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition-all"
                        />
                        ...
                        <div className="space-y-6 animate-in slide-in-from-bottom-4">
                            ...
                            <div className="space-y-3">
                                ...
                                {result.matches.length === 0 ? (
                                    <div className="text-slate-500 text-sm text-center py-4 bg-slate-900 rounded-lg border border-slate-800">
                                    ...
                                    result.matches.sort((a,b) => b.confidence - a.confidence).map((match, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className="bg-slate-800/50 hover:bg-slate-800 border border-slate-700 hover:border-cyan-500/50 rounded-xl p-4 transition-all flex items-start gap-4">
                                                <div className="bg-slate-950 p-3 rounded-lg flex flex-col items-center justify-center min-w-[80px] border border-slate-800 text-cyan-400 font-mono">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between mb-1">
                                                        <h4 className="font-bold text-white flex items-center gap-2">
                                                            ...
                                                            {camDetails && <span className="text-[10px] font-mono text-slate-500 bg-slate-950 px-2 py-0.5 rounded">{camDetails.id}</span>}
                                                        </h4>
                                                    </div>
                                                    <p className="text-sm text-slate-400 mb-3">{match.explanation}</p>
                                                    <button 
                                                        onClick={() => {
